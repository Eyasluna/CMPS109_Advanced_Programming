...
class exit_status {
   private:
      // Act like global variables/functions that can be accessed 
      // without the existence of a "exit_status" object.
      static int status;
   public:
      static void set (int);
      static int get();
...
// Template that overloads operator<< to print out vectors 
...
// Overloads operator<< to print word_range
template <typename iterator>
